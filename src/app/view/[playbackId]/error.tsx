...
export default function ViewerError({
  ...
  useEffect(() => {
    ...
    toast.error('Livestream page failed to load. Please try again.');
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-black via-gray-950 to-black px-6 text-center text-white">
      <h2 className="text-2xl font-bold">Viewer unavailable</h2>
      <p className="mt-3 max-w-md text-sm text-gray-300">
        An error occurred while loading this livestream page.
      </p>
      <button
        type="button"
        onClick={reset}
